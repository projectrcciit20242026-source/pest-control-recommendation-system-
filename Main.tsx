...
function validateField(
  ...
  const result = LoginSchema.pick({ [field]: true } as any).safeParse({
    [field]: value,
  });
  if (!result.success) {
...
export default function Login(): React.JSX.Element {
  ...
  const { name, phone, password, language, loading, rememberMe, errors, touched } =
    formState;
  ...
  const handleFieldChange = (field: "name" | "phone" | "password", value: string): void => {
  ...
  const handleLogin = async (
    ...
    const result = LoginSchema.safeParse({ name, phone, password });
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-3 mb-8 lg:hidden">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Pest Detection & Recommendation System</p>
            </div>
          </div>
